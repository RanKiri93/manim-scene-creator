...
function makeDefaultStep(kind: string): PosStep {
  switch (kind) {
    case 'next_to': return { kind: 'next_to', refKind: 'line', refId: null, dir: 'DOWN', buff: 0.3 };
...
export default function PositionStepsEditor({ steps, onChange, currentItemId }: PositionStepsEditorProps) {
  ...
  const add = () => {
    onChange([...steps, { kind: 'next_to', refKind: 'line', refId: null, dir: 'DOWN', buff: 0.3 }]);
  };
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <label className="text-[10px] text-slate-400">
          ...
          <select
            value={step.dir}
            onChange={(e) => onChange({ ...step, dir: e.target.value as ManimDirection })}
